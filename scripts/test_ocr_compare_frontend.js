...

{
  ...
  assert(/<div class="mathpix-top-actions"[\s\S]*id="runPageMathpixButton"[\s\S]*id="runBookMathpixButton"[\s\S]*id="stopBookMathpixButton"[\s\S]*<\/div>\s*<div class="accepted-download-actions"/.test(ocrCompareHtml), "page and book Mathpix actions should be grouped separately from accepted download actions");
  assert(/<div class="accepted-download-actions"[\s\S]*id="downloadAcceptedCorrectedButton"[\s\S]*<\/div>/.test(ocrCompareHtml), "accepted download should live in the right-side accepted download group");
  assert(ocrCompareCss.includes(".mathpix-top-actions"));
  assert(ocrCompareCss.includes(".accepted-download-actions"));
  assert(ocrCompareCss.includes("justify-content: space-between;"));
  assert(ocrCompareCss.includes(".mathpix-page-action-button"));
  assert(ocrCompareCss.includes(".mathpix-book-action-button"));
  assert(ocrCompareCss.includes(".mathpix-book-stop-button"));
  assert(source.includes('els.runPageMathpixButton?.addEventListener("click", recognizeCurrentPageWithMathpix)'));
  assert(source.includes("function pageMathpixDraftDisabledReason"));
...
const adjacentImageSegments = JSON.parse(
  call(`(() => {
    state.currentPage = 27;
    state.ocrPatches = [];
    state.mineruBlockOverrides.clear();
    state.mathpixBlockDrafts.clear();
    state.reviewNeedsCorrection.clear();
    state.mineruInfo = {
      pdf_info: Array.from({ length: 27 }, (_unused, index) => index === 26 ? {
        page_size: [1000, 1200],
        para_blocks: [
          {
            type: "image",
            bbox: [120, 620, 470, 980],
            lines: [{ bbox: [120, 620, 470, 980], spans: [{ image_path: "fig-2-7.jpg" }, { content: "图2.7" }] }]
          },
          {
            type: "image",
            bbox: [500, 625, 850, 982],
            lines: [{ bbox: [500, 625, 850, 982], spans: [{ image_path: "fig-2-8.jpg" }, { content: "图2.8" }] }]
          }
        ]
      } : { page_size: [1000, 1200], para_blocks: [] })
    };
    return JSON.stringify(reviewSegmentsForPage(27).map((segment) => ({
...
assert.strictEqual(adjacentImageSegments.length, 1, "same-row adjacent image blocks should merge into one review segment");
...
const noiseSeparatedImageSegments = JSON.parse(
  call(`(() => {
    state.currentPage = 29;
    state.ocrPatches = [];
    state.mineruBlockOverrides.clear();
    state.mathpixBlockDrafts.clear();
    state.reviewNeedsCorrection.clear();
    state.mineruInfo = {
      pdf_info: Array.from({ length: 29 }, (_unused, index) => index === 28 ? {
        page_size: [1000, 1200],
        para_blocks: [
          {
            type: "image",
          ...
          {
            type: "text",
            bbox: [360, 1030, 430, 1060],
            lines: [{ bbox: [360, 1030, 430, 1060], spans: [{ content: "- 12 ●" }] }]
          },
          {
            type: "image",
            bbox: [500, 625, 850, 982],
            lines: [{ bbox: [500, 625, 850, 982], spans: [{ image_path: "fig-2-8-right.jpg" }, { content: "图2.8" }] }]
          }
        ]
      } : { page_size: [1000, 1200], para_blocks: [] })
    };
    return JSON.stringify(reviewSegmentsForPage(29).map((segment) => ({
      blockIndex: segment.blockIndex,
      blockIndexes: segment.blockIndexes,
      bbox: segment.bbox,
      kind: segment.kind,
...
assert.strictEqual(noiseSeparatedImageSegments.length, 1, "image blocks separated by figure label and page-number noise should merge into one review segment");
assert.strictEqual(noiseSeparatedImageSegments[0].kind, "image");
assert.deepStrictEqual(noiseSeparatedImageSegments[0].blockIndexes, [0, 1, 2, 3]);
assert(noiseSeparatedImageSegments[0].markdown.includes("图2.7") && noiseSeparatedImageSegments[0].markdown.includes("图2.8"), "noise-separated image merge should keep figure labels");
assert(!noiseSeparatedImageSegments[0].markdown.includes("12"), "page-number noise between image blocks should not remain in merged image markdown");
